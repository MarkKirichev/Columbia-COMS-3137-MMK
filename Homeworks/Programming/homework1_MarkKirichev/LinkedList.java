...
public class LinkedList<T> implements Iterable<T> {
  ...
  /********* ADD YOUR SOLUTIONS HERE *****************/  
    ...
    public void flipPairs() {
        // write this method for problem 2.

		// first we need to check the edge cases where this.size is 0 or 1
		if(this.size == 0 || (this.size - 1) == 0) {
			return;
		}

		final int numberOfFlips = (this.size % 2 == 0) ? (this.size >> 1) : ((this.size - 1) >> 1);

		Node<T> current = this.beginMarker.next;
		Node<T> prevPtr = null;
		Node<T> nextPtr = null;
		for(int i = 0; i < numberOfFlips; ++i) {
			prevPtr = current;      // defining the "previous" pointer to point to the first number of the pair
			nextPtr = current.next; // defining the "next" pointer to point to the second number of the pair
			current = nextPtr.next; // moving the current marker 2 steps ahead i.e. onto the next pair

			// Switching pair-wise
			nextPtr.prev = prevPtr.prev;
			if(prevPtr.prev != null) { // taking care of the edge case in the beginning
				prevPtr.prev.next = nextPtr;
			}

			prevPtr.prev = nextPtr;
			prevPtr.next = nextPtr.next;
			if(nextPtr.next != null) { // taking care of the edge case in the end
		...
		while(prevPtr.prev != null) { // making sure beginMarker points to the right location in memory
			prevPtr = prevPtr.prev;
		}

		this.beginMarker = prevPtr;
    }
    ...
    public void reverse() {
		...
		// if we're not in an edge case we'll need to start
		// at the list's end and return backward while concurrently
		// changing the pointers' directions in each pair

		Node<T> temp = null; // using a temp variable for help with swapping
		this.endMarker = this.beginMarker;
		Node<T> current = this.beginMarker;

		while (!(current == null)) { // implemeting the actual swapping
			temp = current.prev;
		...
		if(!(temp == null)) { // last check to ensure that the beginMarker point to the right node
			this.beginMarker = temp.prev;
		}
    }
